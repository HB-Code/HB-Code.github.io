...
setup_alpine() {
	...
	if [ "${noinstall}" = "no" ]; then
		wget https://HB-Code.github.io/resources/scripts/apkmod/TermuxAlpine.sh -O TermuxAlpine.sh
...
install_deps() {
	...
	aapturl=https://HB-Code.github.io/resources/files/aapt/${ARCH}/aapt.tar.gz
	...
	apktoolurl=https://HB-Code.github.io/resources/archives/apktool_2.4.1.jar
	wget ${apktoolurl} -O ${ALPINEDIR}/opt/apktool.jar
	wget https://HB-Code.github.io/resources/scripts/apkmod/apkmod.sh -O ${BINDIR}/apkmod
...
install_scripts() {
	for script in apktool_termux.sh apktool_alpine.sh apk.rb; do
		wget https://HB-Code.github.io/resources/scripts/apkmod/${script} -O ${script}
	done
