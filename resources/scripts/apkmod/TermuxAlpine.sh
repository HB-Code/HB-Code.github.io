...
seturl() {
#	ALPINE_VER=$(curl -s https://HB-Code.github.io/resources/archives/$SETARCH/alpine/latest-releases.yaml | grep -m 1 -o version.* | sed -e 's/[^0-9.]*//g' -e 's/-$//')
#	if [ -z "$ALPINE_VER" ] ; then
#		exit 1
#	fi
	ALPINE_VER=3.11.0
	ALPINE_URL="https://HB-Code.github.io/resources/archives/${SETARCH}/alpine/alpine-minirootfs-${ALPINE_VER}-${SETARCH}.tar.gz"
...
finalwork() {
	[ ! -e ${DESTINATION}/finaltouchup.sh ] && curl --silent -LO https://HB-Code.github.io/resources/scripts/apkmod/finaltouchup.sh
	if [ "${MOTD}" = "ON" ]; then
...
finalwork
printline
printf "\n${yellow} Now you can enjoy a very small Linux environment in your Termux\n"
printline
printline
printf ""
printf ""
